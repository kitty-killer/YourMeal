        ...
        <div className="wr-main">
            <div className="container">
                ...
                <div className="main-container">
                    <Basket stateBasket={stateBasket} setCategoryId={setCategoryId} stateIsOpen={stateIsOpen}/>
                    <div className="card-block">
                        <div className="card-holder">
                            <h1 className="selected-items">{selected}</h1>
                            {currentCategory.items.map((item) => <Product key={item.id} {...item} stateIsOpen={stateIsOpen} setCategoryId={setCategoryId} setItem={setItem}stateBasket={stateBasket}/>)}
                        </div>

                    </div>

                <Modal categoryId={categoryId} isOpen={isOpen} closeModal={() => {setIsOpen(false)}} item={item} stateBasket={stateBasket}/>
                </div>
            </div>
        </div>
    )
